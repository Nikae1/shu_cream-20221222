	...
	public ArrayList<UserBean> selNote(Connection connection, UserBean user){
		...
		try {
			...
			if(this.rs.isBeforeFirst()) {	
				...
				note.setGoodsPrice(this.rs.getInt("GoodsPrice"));
				note.setGoodsQuantity((this.rs.getInt("GoodsQuantity")));
				...
				note.setTranPlace(this.rs.getNString("TranPlace"));
...
//	}
	...
	public int insNote(Connection connection, UserBean user) {
		...
		String ins = "INSERT INTO TRDBA.NT(NT_CSID, NT_NTCODE,NT_GOODS, "
				+ "NT_COUNT,NT_PRICE, NT_NOTE,NT_DATE, NT_TRPL, NT_TRWAY, "
				+ "NT_NOTEST, NT_NOTETRST, NT_GOSTATE,NT_PLBIG, NT_PLSMALL, NT_GSBIG, NT_GSSMALL) "
				+ "VALUES(?,?,?,?,?,?,DEFAULT, "
				+ "?,?,?,?,?,?,?,?,?)";
		...
		try {
			...
			this.ps.setNString(3, user.getNoteList().get(0).getGoodsName());
			this.ps.setInt(4, user.getNoteList().get(0).getGoodsQuantity());
			this.ps.setInt(5, user.getNoteList().get(0).getGoodsPrice());
			this.ps.setNString(6, user.getNoteList().get(0).getConntents());
			this.ps.setNString(7, user.getNoteList().get(0).getNoteDate());
			this.ps.setNString(8, user.getNoteList().get(0).getTranPlace());
			this.ps.setNString(9, user.getNoteList().get(0).getTranMethod());
			this.ps.setNString(10, user.getNoteList().get(0).getCateList().get(0).getCg_Code());
			this.ps.setNString(11, user.getNoteList().get(0).getCateList().get(0).getCg_Code());
			this.ps.setNString(12, user.getNoteList().get(0).getCateList().get(0).getCg_Code());
			this.ps.setNString(13, user.getNoteList().get(0).getCateList().get(0).getPlaceList().get(0).getPlaceBig());
			this.ps.setNString(14, user.getNoteList().get(0).getCateList().get(0).getPlaceList().get(0).getPlaceSma());
			this.ps.setNString(15, user.getNoteList().get(0).getCateList().get(0).getGoodsList().get(0).getGs_Big());
			this.ps.setNString(16, user.getNoteList().get(0).getCateList().get(0).getGoodsList().get(0).getGs_Small());
	...
	final ArrayList<UserBean> getMaxNoteCode(Connection connection, UserBean user) {
		ArrayList<UserBean> userList = null;
		String qur = "SELECT NVL(MAX(NT_NTCODE), '000000001') AS MAXNOTECODE FROM TRDBA.NT";
		...
		try {
			...
			if(this.rs.isBeforeFirst()) {
				userList = new ArrayList<UserBean>();
				while (this.rs.next()) {
					user.getNoteList().get(0).setNoteCode(this.rs.getNString("MAXNOTECODE"));
					userList.add(user);
				}
			}
